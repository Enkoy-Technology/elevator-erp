    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          'Pricing is the company’s ETB price sheet, not a cost model, and the formula that turns the sheet into a price is the company’s own: Settings → Pricing → List-price formula. The starter is “Base price + (N − refN) × perStop + ((C − refC) / kgStep) × perKg”, where Base price, refN, refC, perStop, perKg and kgStep are the product’s figures from Products & prices — kgStep being the kilograms one perKg step covers, 100 on the passenger class and 1,000 on car and goods lifts, so a passenger lift prints as the sheet writes it: 7,000,000 + (N − 10) × 80,000 + ((C − 630) / 100) × 100,000 — N is the stops (a car stacking lift’s parking levels, L), C the rated capacity in kg and Rise the travel in metres. It is evaluated exactly as written, per line, before margin and VAT — write “max(0, N − refN)” if a lift below the base machine should cost the base rather than less. Brackets, + − × ÷, thousands separators, unit suffixes (630kg) and max(), min(), round() are understood; a formula that does not parse or gives a negative price is refused on save. A product with its own formula ignores the company one.',
          'If the sheet already includes VAT — 7,000,000 is what the customer pays, VAT in — set Settings → Pricing → VAT included in the price list to 15. The calculator then divides the VAT out of the list figure before margin and VAT are applied (6,086,956.52 net + 913,043.48 VAT = 7,000,000), shows the amount as “Less VAT included in list” in the breakdown, and the offer’s total equals the sheet. Leave it blank for an ex-VAT sheet.',
        ],
        facts: [
          {
        ...
        flows: [
          {
            ...
            steps: [
              'Product base price',
              'List-price formula (N, C)',
              '+ margin %',
              '+ VAT %',
              'Grand total',
            ],
            note: 'The breakdown splits the formula’s result into what the stops added (evaluated at 630 kg) and what the capacity added on top, so it stays honest for any formula the company writes.',
          },
        ],
        rules: [
