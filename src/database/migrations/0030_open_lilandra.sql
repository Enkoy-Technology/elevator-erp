CREATE TYPE "public"."quote_status" AS ENUM('DRAFT', 'PENDING_APPROVAL', 'APPROVED', 'REJECTED', 'EXPIRED', 'CONVERTED_TO_PROFORMA');--> statement-breakpoint
...
ALTER TABLE "quotations" ADD CONSTRAINT "quotations_rate_version_id_rate_versions_id_fk" FOREIGN KEY ("rate_version_id") REFERENCES "public"."rate_versions"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
