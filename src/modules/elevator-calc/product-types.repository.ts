...
import { productTypes } from '../../database/schema';
...
export const DEFAULT_PRODUCT_TYPES: readonly {
  ...
  { code: 'PASSENGER', name: 'Passenger elevator', basePriceEtb: '7000000.00', perStopEtb: '80000.00', perKgEtb: '1000.00', liftGeometry: true },
  { code: 'HOSPITAL', name: 'Hospital elevator', basePriceEtb: '7000000.00', perStopEtb: '80000.00', perKgEtb: '1000.00', liftGeometry: true },
  { code: 'PANORAMIC', name: 'Panoramic elevator', basePriceEtb: '8000000.00', perStopEtb: '80000.00', perKgEtb: '1000.00', liftGeometry: true },
  { code: 'HOME', name: 'Home elevator', basePriceEtb: '8000000.00', perStopEtb: '80000.00', perKgEtb: '1000.00', liftGeometry: true },
  { code: 'CARGO', name: 'Cargo elevator', basePriceEtb: '8000000.00', perStopEtb: '80000.00', perKgEtb: '1000.00', liftGeometry: true },
  { code: 'CAR_LIFT', name: 'Car lift', basePriceEtb: '12000000.00', perStopEtb: '80000.00', perKgEtb: '1000.00', liftGeometry: true },
  { code: 'CAR_PLATFORM_LIFT', name: 'Car platform lift', basePriceEtb: '3200000.00', perStopEtb: '0.00', perKgEtb: '0.00', liftGeometry: false },
  { code: 'ESCALATOR', name: 'Escalator', basePriceEtb: '6000000.00', perStopEtb: '0.00', perKgEtb: '0.00', liftGeometry: false },
];
...
export class ProductTypesRepository {
  ...
  list(tenantId: string): Promise<ProductTypeRecord[]> {
    return this.tenantDb.withTenant(tenantId, async (tx) => {
      ...
      const [any] = await tx.select({ id: productTypes.id }).from(productTypes).limit(1);
      if (any) {
        return rows;
      }
      return tx
        .insert(productTypes)
        .values(DEFAULT_PRODUCT_TYPES.map((p, i) => ({ tenantId, ...p, sortOrder: i })))
        .onConflictDoNothing()
        .returning();
    });
  }

  async findByCode(tenantId: string, code: string): Promise<ProductTypeRecord | null> {
    const rows = await this.list(tenantId);
    return rows.find((row) => row.code === code) ?? null;
  }

  async create(tenantId: string, input: ProductTypeInput): Promise<ProductTypeRecord> {
    return this.tenantDb.withTenant(tenantId, async (tx) => {
      ...
      if (clash && clash.deletedAt === null) {
        throw new ConflictException(`A product with the code ${code} already exists`);
      }
      if (clash) {
      ...
      const [row] = await tx
        .insert(productTypes)
        .values({ tenantId, code, ...input, sortOrder: (max?.sortOrder ?? -1) + 1 })
        .returning();
      if (!row) {
